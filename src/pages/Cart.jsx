      ...
      {cartItems.length === 0 ? (
        ...
        <div className="space-y-6">
          <ul className="divide-y divide-gray-300">
            {cartItems.map((item) => (
              <li key={`${item.id}-${item.selectedQuantity}`} className="flex items-center justify-between py-4">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-700 mt-1">
                      <strong>Selected Quantity:</strong> {item.selectedQuantity}
                    </p>

                    {/* Quantity Controls */}
                    <div className="flex items-center space-x-3 mt-2">
                      <button
                        onClick={() => decreaseQuantity(item.id, item.selectedQuantity)}
                      ...
                      <button
                        onClick={() => increaseQuantity(item.id, item.selectedQuantity)}
                        className="bg-gray-200 hover:bg-gray-300 text-gray-800 px-3 py-1 rounded-lg"
                      >
                        +
                      </button>
